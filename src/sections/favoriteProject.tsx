...
const FavoriteProject = () => {
    ...
    return (
        <section className="bg-[#1B1820] pb-2 rounded-sm text-white">
            ...
            {loading && (
                <ProjectStateNotice
                    title="Loading featured project"
                    message="Fetching the project you want to highlight."
                />
            )}

            {!loading && error && (
                <ProjectStateNotice
                    title="Featured project unavailable"
                    message={error}
                    tone="error"
                />
            )}

            {!loading && !error && !favorite && (
                <ProjectStateNotice
                    title="Featured project missing"
                    message="WaveSpacer was not found in the project list."
                    tone="empty"
                />
            )}
